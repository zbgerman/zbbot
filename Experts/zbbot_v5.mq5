...
input double inpporcentajeRiesgo = 2; //Porcentaje de Riesgo
...
input color Color_Bullish_Current_CE = C'132,132,0';
input color Color_Bearist_Current = C'255,113,137';
input color Color_Bearist_Current_CE = clrRed;
...
int intervalC = 90000; //Se cambia cuando verifico si esta en backtesting a 15 segundso
int RejectionBlockInterval = 900; 
int NoticiasInterval = 180;
...
//--- Desactivar autoajuste y forzar margen
  ...
  Print("Inicio zbbot : ",_Symbol);
   ...
   DrawBarFractals(_Period, 1000, 30, "1");// Fractal para el fibo de M15 y 1000 velas y 30 para detectar el fibo 
   DrawBarFractals(VGtime_Frame_Fractal, 1000, 30, "2");// Fractal para el fibo de M15 y 1000 velas y 30 para detectar el fibo
...
//Utilidad
   ...
   porcentajeUtilidad.Text(porcentajeUtilidad1);
...
//--- Tendencia M3 - M15 - H1 - H5 - D1
   m3Btn.Create(0, M3_BTN_NAME, 0, 0 , 205, 0, 0);
   ...
   m15Btn.Create(0, M15_BTN_NAME, 0, 0 , 225, 0, 0);
   ...
   h1Btn.Create(0, H1_BTN_NAME, 0, 0 , 245, 0, 0);
   ...
   h4Btn.Create(0, H4_BTN_NAME, 0, 0 , 265, 0, 0);
   ...
   d1Btn.Create(0, D1_BTN_NAME, 0, 0 , 285, 0, 0);
...
//Print("zbgerman ", TimeCurrent(), "timerText:",timerText);
   ...
   if(now - lastActionModelo2022 >= intervalModelo2022M5) // Cada 900 segundosmo
   {
      ...
      if(sparam=="Resistencia" || sparam=="Soporte" )
        {
            ...
            { 
               ...
               if(clickTime < clickTimeMemory + 300)
               {
                  if (lvcolor == clrYellow)
                  {
                     lvcolor = ObjectGetInteger(0,"Resistencia",OBJPROP_COLOR);
                     ObjectSetInteger(0, "Resistencia", OBJPROP_STYLE, STYLE_SOLID);
                     ObjectSetInteger(0, "Resistencia", OBJPROP_COLOR, clrLimeGreen);
                  ...
                  else
                  {
                     ...
                     ObjectSetInteger(0, "Resistencia", OBJPROP_WIDTH, 1);
                     ObjectSetInteger(0, "Soporte", OBJPROP_COLOR, clrLimeGreen);
...
void ProgramarCompraVenta()
  {
      ...
      else // Existe, actualizarla
      {
        ...
        ObjectSetInteger(0, midPriceName, OBJPROP_COLOR,clrLime);
...
//         }   
        ...
        TextLabel = "PF:" + DoubleToString(profit_money_sell,2) + " Loss:" + DoubleToString(loss_money_sell,2)  + " Lots:" + DoubleToString(lvlote,2) + " RR:" + DoubleToString(lvratio,2);// +  " P" + DoubleToString(lvpuntosprofit_sell,1) + " L" + DoubleToString(lvpuntosloss_sell,1) ; 
        ...
        ObjectSetInteger(current_chart_id, vllabel,OBJPROP_SELECTABLE,true);
...
//         }
         ...
         TextLabel = "PF:" + DoubleToString(profit_money_buy,2) + " Loss:" + DoubleToString(loss_money_buy,2) + " Lots:" + DoubleToString(lvlote,2) + " RR:" + DoubleToString(lvratio,2);//  +  " P:" + DoubleToString(lvpuntosprofit_buy,2) + " L:" + DoubleToString(lvpuntosloss_buy,2) ; //+ "  SL : " + NormalizeDouble(PipsSL,2) + " TP : " + NormalizeDouble(PipsTP,2) ;
...
void Alarmas()
{
   ...
   if(VGcontadorAlertasAlcista >= 2)//  && VGTendencia_interna_H4 == "Alcista" && VGTendencia_interna_H1 == "Alcista" )
   {
      ...
      if (VGPorcentaje > inpporcentajeRetroceso && VGmodelo2022 == false && VGtradedia == 0 && MQLInfoInteger(MQL_TESTER))
      {
         VGtradedia++;
   ...
   if(VGcontadorAlertasBajista >= 2)//  && VGTendencia_interna_H4 == "Bajista" && VGTendencia_interna_H1 == "Bajista" )
   {
   ...
   if(VGTendencia_externa == "Bajista")//VGcontadorAlertasBajista > 1 )//&& VGHoraNewYork.hour >= 09 && VGHoraNewYork.hour <= 10)// && Bid > VGMidPrice && VGContadorAlertasZona == 0) //&&  VGTendencia_interna == "Bajista" && VGContadorAlertasZona == 0)
   {
      ...
      //VGPorcentaje = (Bid - VGvalor_fractal_bajo_5 ) / (VGvalor_fractal_alto_5 - VGvalor_fractal_bajo_5) * 100;
      if(VGPorcentaje >=50 && VGContadorAlertasZona == 0)
      {
   ...
   if(VGTendencia_externa == "Bajista" && VGContadorAlertasOte == 0)//VGcontadorAlertasBajista > 0 && VGPorcentaje > 61 && VGContadorAlertasOte == 0 )// && VGTendencia_interna == "Bajista" )
   {
      VGPorcentaje = (Bid - VGSoporte ) / (VGResistencia - VGSoporte) * 100;
      LVPosibleTrade = _Symbol + " :Zona OTE !!!" + DoubleToString(VGSoporte,2) + " Server : " + TimeCurrent() + " Local : " + TimeLocal() ;//  " Lote " + NormalizeDouble(lvlote,2) + " - " +  " Ratio : " + NormalizeDouble(lvratio,2);  
      if(VGPorcentaje >=61 && VGContadorAlertasZona == 0)
      {
         textohablado("\"Zona OTE " + _Symbol +\"",true);
         VGContadorAlertasOte++;
      }   
   }    

   if(VGTendencia_externa == "Alcista" && VGContadorAlertasOte == 0)//VGcontadorAlertasAlcista > 0 && VGPorcentaje > 61 && VGContadorAlertasOte == 0 )// && VGTendencia_interna == "Alcista" )
   {
      VGPorcentaje = (VGResistencia - Bid) / (VGResistencia - VGSoporte) * 100;
      LVPosibleTrade = _Symbol + " :Zona OTE !!!" + DoubleToString(VGSoporte,2) + " Server : " + TimeCurrent() + " Local : " + TimeLocal() ;//  " Lote " + NormalizeDouble(lvlote,2) + " - " +  " Ratio : " + NormalizeDouble(lvratio,2);  
      //SendNotification(LVPosibleTrade);
      //Alert(LVPosibleTrade);
      //TextToSpeech("\"Zona OTE " + _Symbol +\"");
      if(VGPorcentaje >=61 && VGContadorAlertasZona == 0)
      {
         textohablado("\"Zona OTE " + _Symbol +\"",true);
         VGContadorAlertasOte++;
   ...
   if(VGTendencia_externa == "Alcista")// VGcontadorAlertasAlcista > 1)// && VGHoraNewYork.hour >= 09 && VGHoraNewYork.hour <= 10) //&& Bid < VGMidPrice && VGContadorAlertasZona == 0) //&& VGTendencia_interna == "Alcista" 
   {
      VGSoporte = ObjectGetDouble (0,"Soporte",OBJPROP_PRICE);
      VGPorcentaje = (VGResistencia - Bid) / (VGResistencia - VGSoporte) * 100;
  ...
  if (lvpuntosvelaanterior > lvpuntosvela)// && ContadorvelaZB == 0)
  {
   ...
   // Eliminar el objeto si ya existe
      ...
      if (!ObjectCreate(0, arrow_name, OBJ_ARROW, 0, time, price))
      {
         ...
         ObjectSetInteger(0, arrow_name, OBJPROP_SELECTABLE, true);                   
         ObjectSetInteger(0, arrow_name, OBJPROP_SELECTED, true);
...
void CrearLineaPromedio()
{
    ...
    ObjectSetInteger(0, NombreLineaPromedio, OBJPROP_SELECTABLE, true); 
      ...
      if(low0 > high2 && IsFVGReplenished_Bullish(timeframe, i, low0, high2))
        {
            ...
            {
              if(VGlowestLow < low0)// && VGTendencia_interna_H4 ==  "Alcista")
              {
                  VGcumplerregla = true;
                  //Print("german : contadorFVGbullish = ",contadorFVGbullish,  " low : ", low0, " high2 :",high2, " VGlowestLow : ",VGlowestLow);
      ...
      if(high0 < low2 && IsFVGReplenished_Bearish(timeframe, i,low2, high0))
        {
            ...
            if( fvgwidh == 9)//Para verificar si el precio esta actualmente en un FVG
            {
              ...
              //Print("german : contadorFVGbearish = ",contadorFVGbearish,  " low2 : ", low2, " high0 :",high0, " VGhighestHigh : ",VGhighestHigh );
              if(VGhighestHigh > high0)// && VGTendencia_interna_H4 ==  "Bajista")
              {
                  VGcumplerregla = true;
...
double CalculateLotSize(double entry_price, double stop_loss, double risk_percent )
{
   ...
   double pointSize   = SymbolInfoDouble(Symbol(), SYMBOL_POINT);
   double pointValue   = tick_value * (Point() / tick_size);                       // Valor por punto   
...
//             return;
         ...
         ObjectSetInteger(0,obj_name_text,OBJPROP_SELECTABLE,true);
...
string GetServerTimeNY(string horaNY)
{
    ...
    int serverOffset = (serverTime - gmtTime) / 3600; // Diferencia en horas
...
//Segunda version
   ...
   double lvpuntos_vela =MathAbs(iOpen(_Symbol,PERIOD_M1,0) - iClose(_Symbol,PERIOD_M1,0)) / Puntos ;
...
//         }
         ...
         if(iOpen(_Symbol,Time_Frame_M2022,1) > iClose(_Symbol,Time_Frame_M2022,1) && VGcontadorAlertasBajista <= 0)
         {
          ...
          if(lvclose < valor_fractal_bajo_1 && VGcontadorAlertasBajista <= 0)// && VGcontadorAlertasBajista == 0) // && VGTendencia_interna_M3 == "Alcista" && VGPorcentaje_fibo_M3 < 50 &&  VGPorcentaje_fibo_M3 > 30)// && lvpuntos_vela > tolerance_value )// && VGTendencia_interna == "Bajista")// && valor_fractal_alto_1 > valor_fractal_alto_2)
          {
               ...
               {
                  lvmensaje = "\"Oportunidad de Venta con vela grande : " +  _Symbol + " " + lv_timeframe +  \"";
                  textohablado(lvmensaje, true);
               }
               else
               {
                  lvmensaje = "\"Oportunidad de Venta con vela : " +  _Symbol + " " + lv_timeframe +  \"";
...
// COMPRAS         
         ...
         //{
            ...
            if( lvclose > valor_fractal_alto_1  && VGcontadorAlertasAlcista <= 0 )// && VGTendencia_interna_M3 == "Bajista" && VGPorcentaje_fibo_M3 < 50 &&  VGPorcentaje_fibo_M3 > 30)// && lvpuntos_vela > tolerance_value ) //&& valor_fractal_bajo_1 < valor_fractal_bajo_2
            {
               ...
               {
                  lvmensaje = "\"Oportunidad de compra con vela : " +  _Symbol + " " + lv_timeframe +  \"";
                  textohablado(lvmensaje, false);
               }
...
//      }
   ...
   if(lvflag == "1")
   {
      ...
      if(VGTendencia_interna == "Alcista")
      {
         ...
         //VGPorcentaje = (lvresistencia - Bid) / (lvresistencia - lvsoporte) * 100;
      }
      else
      {
         //VGPorcentaje = (Bid - lvsoporte ) / (lvresistencia - lvsoporte) * 100;
      }
   }
...
void DetectImmediateRebalancePattern(ENUM_TIMEFRAMES lv_timeframes)
{
   ...
   // --- Obtenemos los precios de las velas relevantes ---
   ...
   {
       if(!ObjectCreate(0, ir_name, OBJ_TREND, 0, iTime(_Symbol,lv_timeframes,3) , low_vela1))
       {
           Print("Error al crear la línea de tendencia: ", GetLastError());
           return;
       }
