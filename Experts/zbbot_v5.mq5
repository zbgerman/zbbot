...
double VGvalor_fractal_alto_M3;
double VGvalor_fractal_bajo_M3;

double VGvalor_fractal_alto_M15;
double VGvalor_fractal_bajo_M15;
...
//    CalculateNDOG_LastWeekTWT();
   ...
   if ( Period() >= PERIOD_M3 && Period() < PERIOD_M15)// || Period() == PERIOD_M1 || Period() == PERIOD_M2 || Period() == PERIOD_M3 || Period() == PERIOD_M5 || Period() == PERIOD_M10 || Period() == PERIOD_M20 ) //comparo si es PERIOD_D1
   {
     VGTime_Frame_HT = PERIOD_M15;
     //Print("VGTime_Frame_HT:",VGTime_Frame_HT);
   }
   if ( Period() < PERIOD_M3)// || Period() == PERIOD_M1 || Period() == PERIOD_M2 || Period() == PERIOD_M3 || Period() == PERIOD_M5 || Period() == PERIOD_M10 || Period() == PERIOD_M20 ) //comparo si es PERIOD_D1
   {
     VGTime_Frame_HT = PERIOD_M3;
     //Print("VGTime_Frame_HT:",VGTime_Frame_HT);
   }
...
//      } 
      ...
      if (VGminutos_noticias > 0)
      {
            ...
            newsButton.ColorBorder(clrWhite);
      }
      else
      {
            newsButton.Hide();
...
//Print("zbgerman ", TimeCurrent(), "timerText:",timerText);
   ...
   if(now - lastActionNoticias >= NoticiasInterval) // 180 segundos
   {
    ...
    // Recorrer todos los objetos en el gráfico actual
    for(int i = 0; i < ObjectsTotal(0, 0, -1); i++)
    {
        string name_object = ObjectName(0, i, 0, -1);
      ...
      if(buyStopBtn.MouseX())
        {
            ...
            for (int i=1; i <= lvnumerocompras; i++)
            {
               //MiTrade.BuyStop(lvlote,VGMinimo2,_Symbol,VGMinimo1,VGMaximo2,ORDER_TIME_GTC,0,"Buy Stop zb");//Con Stop Loss
               MiTrade.BuyStop(lvlote,VGMinimo2,_Symbol,0,VGMaximo2,ORDER_TIME_GTC,0,"Buy Stop zb");//Sin stop Loss
      ...
      if(sellStopBtn.MouseX())
        {
            ...
            for (int i=1; i <= lvnumerocompras; i++)
            {
               //MiTrade.SellStop(lvlote,VGMaximo1,_Symbol,VGMaximo2,VGMinimo1,ORDER_TIME_GTC,0,"Sell Stop zb");//Con Stop Loss
               MiTrade.SellStop(lvlote,VGMaximo1,_Symbol,0,VGMinimo1,ORDER_TIME_GTC,0,"Sell Stop zb");//Sin stop Loss
...
//   {
//
//      double     vlhighestHigh = iHigh(Symbol(), PERIOD_M15, iHighest(Symbol(), PERIOD_M15, MODE_HIGH, 30, 0));
//      
//      if ( vlhighestHigh > VGvalor_fractal_alto_M15)
//         return;
//
//
//
//      if (iHigh(_Symbol,PERIOD_M15,1) > VGvalor_fractal_alto_M15)
//      {
//         if (iClose(_Symbol,PERIOD_M15,1) > VGvalor_fractal_bajo_M15)
//         {
//            textohablado("\" Cambio de tendencia Alcista  en M15 "+ _Symbol +\"", true);
//         }
//         else
//         {
//            textohablado("\" Liquidity Sweep   en M3 "+ _Symbol +\"", true);
...
//   }   
//
//   if (VGTendencia_interna_M15 == "Alcista" && VGContadorAlertasZona_M15 > 0)
//   {
//      double     vllowestLow = iLow(Symbol(), PERIOD_M15, iLowest(Symbol(), PERIOD_M15, MODE_LOW, 30, 0));
//      
//      if (vllowestLow < VGvalor_fractal_bajo_M15)
//         return;
//
//
//      if (iLow(_Symbol,PERIOD_M15,1) < VGvalor_fractal_bajo_M15)
//      {
//         if (iClose(_Symbol,PERIOD_M15,1) < VGvalor_fractal_bajo_M15)
//         {
//            textohablado("\" Cambio de tendencia Bajista  en M15 "+ _Symbol +\"", true);
//         }
//         else
//         {
...
//      tp("TP1_Temporal",lvmidprice,vlalto,2,lot);
   ...
   {
//      double lvfractal_bajo =  ObjectGetDouble(0,"Fractal_L_5_1",OBJPROP_PRICE);
//      double lvfractal_alto =  ObjectGetDouble(0,"Fractal_H_5_1",OBJPROP_PRICE);
//      double vlhighestHigh = iHigh(Symbol(), PERIOD_M1, iHighest(Symbol(), PERIOD_M1, MODE_HIGH, 15, 0)); 
//      double vllowestLow = iLow(Symbol(), PERIOD_M1, iLowest(Symbol(), PERIOD_M1, MODE_LOW, 15, 0));     
// 
//      if( VGcontadorAlertasAlcista > 0)// && vlhighestHigh < lvfractal_alto )
//      {
//         ObjectSetDouble(0,"FIBO_3",OBJPROP_PRICE,1,lvfractal_alto);
//         ObjectSetDouble(0,"FIBO_3",OBJPROP_PRICE,0,lvfractal_bajo);
//         ObjectSetDouble(0,"FIBO_3",OBJPROP_PRICE,1,vlhighestHigh);
//      }   
//   
//      if( VGcontadorAlertasBajista > 0)// && vllowestLow > lvfractal_bajo )
//      {
//         ObjectSetDouble(0,"FIBO_3",OBJPROP_PRICE,1,lvfractal_alto);
//         ObjectSetDouble(0,"FIBO_3",OBJPROP_PRICE,0,lvfractal_bajo);
//         ObjectSetDouble(0,"FIBO_3",OBJPROP_PRICE,0,vllowestLow);
//      }   
      //Print("lvfibo_3_1 : ",lvfibo_3_1, " lvfibo_3_0 : ",lvfibo_3_0);
   }
   ...
   if(Bid > lvfibo_3_0)
   {
      VGContadorAlertasZona = 0;
      VGContadorAlertasOte  = 0;
      lvfibo_3_0 = iHigh(Symbol(), PERIOD_M1, iHighest(Symbol(), PERIOD_M1, MODE_HIGH, 15, 0));
      //lvfibo_3_1 = iLow(Symbol(), PERIOD_M1, iLowest(Symbol(), PERIOD_M1, MODE_LOW, 4, 0));
   ...
   if(Bid < lvfibo_3_1 )
   {
      ...
      //lvfibo_3_0 = iHigh(Symbol(), PERIOD_M1, iHighest(Symbol(), PERIOD_M1, MODE_HIGH, 4, 0));
      lvfibo_3_1 = iLow(Symbol(), PERIOD_M1, iLowest(Symbol(), PERIOD_M1, MODE_LOW, 15, 0));
      //ObjectSetDouble(0,"FIBO_3",OBJPROP_PRICE,0,lvfibo_3_0);
      ObjectSetDouble(0,"FIBO_3",OBJPROP_PRICE,1,lvfibo_3_1);
...
void verificar_ordenes_Abiertas()
{
   ...
   if(TotalPosiciones > 0)
   {
      ...
      if (lvcontador <=0)
      {
         ObjectsDeleteAll(0,"BUY_TP1");
         ObjectsDeleteAll(0,"SELL_TP1");
      }   
   }
   else
   {
...
void DrawFVG(ENUM_TIMEFRAMES timeframe, int candlesToCheck, color colorBullis, color colorBearish, int fvgwidh  )
{
   ...
   string name ="";
   string nametimeframe = TimeframeToString(timeframe);;
   string namece = "";
   double high0;
   double low0 ;
   double high2 ;
      ...
      if(low0 > high2 && IsFVGReplenished_Bullish(timeframe, i, low0, high2, contadorFVGbullish))
        {
            ...
            if( fvgwidh == 9 && contadorFVGbullish >= Numero_velas - 1)//Para verificar si el precio esta actualmente en un FVG
            {
              ...
              if(vlbag ==  1 && i >= 3)// && VGTendencia_interna_H4 ==  "Alcista")
              {
                  ...
                  string lvmensaje = "\"BAG ALCISTA !!! : " + " " + _Symbol + "  " + nametimeframe + \"";
                  //textohablado(lvmensaje,true);
                  VGcumplerregla = true;
                  break;
           ...
           if (fvgwidh > 0)
            {
              ...
              name = "FVG_" + nametimeframe + "_Bullish_" + IntegerToString(contadorFVGbullish);
              namece = "FVG_"+ nametimeframe + "_CE_Bullish_" + IntegerToString(contadorFVGbullish);
            }  
      ...
      if(high0 < low2 && IsFVGReplenished_Bearish(timeframe, i,low2, high0, contadorFVGbearish))
        {
            ...
            if( fvgwidh == 9 && contadorFVGbearish >= 5)//Para verificar si el precio esta actualmente en un FVG
            {
              ...
              if(vlbag ==  1 && i >= 3)// && VGTendencia_interna_H4 ==  "Alcista")
              {
                  ...
                  string lvmensaje = "\"BAG BAJISTA !!! : " + " " + _Symbol + "  " + nametimeframe + \"";
                  //textohablado(lvmensaje,true);
                  VGcumplerregla = true;
                  break;
                  //Print("german : contadorFVGbullish = ",contadorFVGbullish,  " low : ", low0, " high2 :",high2, " vlbajo : ",vlbajo, " i : ",i);
         ...
         if (fvgwidh > 0)
         {
            ...
            endTime = iTime(NULL, timeframe, i - fvgwidh);
            name = "FVG_" + nametimeframe + "_Bearish_" + IntegerToString(contadorFVGbearish);
            namece = "FVG_" + nametimeframe + "_CE_Bearish_" + IntegerToString(contadorFVGbearish);
         }   
         //datetime endTime = iTime(NULL, timeframe, 0);
...
//+------------------------------------------------------------------+
//| Script para dibujar macros y kill zone                           |
//+------------------------------------------------------------------+
void DrawMacrosKillzone()
  {
...
//                  }
            ...
            if(timeframe == PERIOD_M1)
            {
              ...
              VGvalor_fractal_alto_M1 = fractal_alto[1];;
              VGvalor_fractal_bajo_M1 = fractal_bajo[1];
            ...
            if(timeframe == PERIOD_M3)
            {
              ...
              VGvalor_fractal_alto_M3 = fractal_alto[1];
              VGvalor_fractal_bajo_M3 = fractal_bajo[1];
            ...
            if(timeframe == PERIOD_M15)
            {
              ...
              VGvalor_fractal_alto_M15 = fractal_alto[1];
              VGvalor_fractal_bajo_M15 = fractal_bajo[1];
...
void CreateButtons()
{
  ...
  if (lvxpos > 0 && lvxpos < 1000 )
   ...
   xPos = xPos - 50;
   if (newsButton.Create(0, NEWS_BTN_NAME, 0, xPos, yPos, xPos + buttonWidth, yPos + buttonHeight));
   {
      ObjectSetString(0, newsButton.Name(), OBJPROP_TOOLTIP, "0 minutos ");
      newsButton.Text("NEWS");
      newsButton.ColorBackground(clrGray);
      newsButton.Color(clrWhite);      
      newsButton.FontSize(FONT_SIZE);
      newsButton.ColorBorder(clrWhite);
   }
